...
class SQLFilterVisitor implements FilterVisitor<String, Map<String, Object>> {
    ...
    String visitEqualsIgnoreCaseFilter(Map<String, Object> params, EqualsIgnoreCaseFilter filter) {
	if (isAlwaysTrue(filter)) {
	    return "TRUE"
	} else {
	    return "UPPER(\${" + filter.name + "}) = UPPER(:" + addParameter(params, filter.attribute) + ")"
	}
    }
